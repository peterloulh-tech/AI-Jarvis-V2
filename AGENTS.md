...
1. **已有资产**：V2 当前实现，以及 AIJARVISV2-17/18 已确认的原作者可复用资产；能直接满足就直接复用，能最小修改满足就修改复用，不新增平行实现。
2. **官方基线**：涉及模型、runtime 或 API 特定行为时，对齐 [`OFFICIAL_BASELINE_INDEX.md`](docs/v2/memory/OFFICIAL_BASELINE_INDEX.md) 的项目锁定 revision；已有基线默认不重复全量联网审计，只有版本变化、信息缺失或实际行为冲突时才重查第一方来源。
