...
Codex 默认在一轮内完成：读取冻结需求、Dashi/Git 状态和真实代码；检查当前官方、原作者与项目资产；完成四方审计并选择最短正确路线；直接实施必要修改；运行与改动直接相关的必要验证；检查 `git diff`/`status`；直接 commit/push；输出供第二审核使用的证据报告。普通小差异在同一轮修完，不因实现细节等待 ChatGPT；已经验证且相关代码和环境未变的能力不重复证明。
