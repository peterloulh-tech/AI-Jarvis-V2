...
V 测试候选锁定为两个独立 profile：`V-C01 = Qwen3-VL-4B-Instruct Q4_K_M + Q8 mmproj`，`V-C02 = MiniCPM-V-4.6 Q4_K_M + F16 mmproj`，共同使用 `llama-server b10369`。运行时固定 [`ggml-org/llama.cpp b10369@6e62ba5`](https://github.com/ggml-org/llama.cpp/tree/6e62ba538478202094edc6c100c782719e310aa3)；Qwen 模型源码固定 [`QwenLM/Qwen3-VL@9658872`](https://github.com/QwenLM/Qwen3-VL/tree/96588727e44c78b25ba03ea03b8e12f7e64fd0da)，base/GGUF 固定为 `ebb281e...` / `1cd86af...`；MiniCPM revision 见候选 lock。当前官方 server 的 inference mode 由一个 `server_context` 持有主模型与多个 slots，直接支持 `--parallel`、continuous batching、multimodal 和 JSON Schema；每轮只加载一个候选，不得同时驻留、切换 router 多实例或用多个进程冒充共享权重。

固定结论仅为双候选静态准入，不预选最终产品模型：官方多媒体数组路径存在，但 b10369 视觉单测仍标有 multiple-images TODO；Windows CUDA release 存在也不代表目标 NVIDIA、8/12GB、完全离线或便携依赖通过。任务 26 必须用同一测试包和共同对照组分别实测两个 profile；任务 27 对通过硬门的候选分别测共享权重 1～3 slots；最终由任务 29 基于动态证据冻结唯一产品模型。V2 只允许补等比留边、提交时风格、`task_id/captured_at/run_generation`、结果校验和单进程全局复位；禁止新增 runtime/server、OCR、分类/摘要模型或在线核心服务。完整候选矩阵、许可边界、淘汰理由和任务 26/27 方案见 [`v-model-runtime-candidate-screening.md`](../requirements/v-model-runtime-candidate-screening.md)。

任务 26 动态纠正（2026-08-12～13）：官方 metadata-only commit `594171a` 把 Qwen GGUF architecture 改成 b10369 不识别的名称；按用户决定改锁同一官方模型的前一文件 `1cd86af...`（`qwen3vl`，SHA `66358c...a0a`），变更准入后成功加载。普通/高光由 schema 强制输出，只有平静允许智能沉默。V-C02 的 6/9 根因进一步定位为字段生成顺序和旧平静 fixture 的事件歧义；改为先生成 level、静默/发言完整互斥 schema、无主体平静图和占位事件拒绝后，Mac/Metal 严格顺序重跑两个 profile。V-C01/V-C02 的 JSON/schema、完整契约、emit、level 均 9/9，硬门均为 `PASS`，每个 profile 调用 9、修复 0。Windows/NVIDIA、8/12GB 与真实语料质量仍为 `UNCONFIRMED`。大型制品普通运行按 `DEC-ARTIFACT-INTEGRITY-01` 只核对路径、文件名与字节数，完整 SHA 仅在准入/变化/最终组包事件执行并复用记录。
