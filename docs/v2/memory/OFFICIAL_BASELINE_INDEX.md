...
任务 26 动态纠正（2026-08-12）：官方 metadata-only commit `594171a` 把 Qwen GGUF architecture 改成 b10369 不识别的名称；按用户决定改锁同一官方模型的前一文件 `1cd86af...`（`qwen3vl`，SHA `66358c...a0a`），变更准入后成功加载。普通/高光改为 schema 强制输出，只有平静允许智能沉默；fixture 同时修正单图误标运动和高光主体遮挡。Mac/Metal 顺序实测 V-C01 完整契约 9/9、硬门 `PASS`；V-C02 required 6/6，但平静沉默 0/3，整体硬门 `FAIL`。Windows/NVIDIA、8/12GB 与真实语料质量仍为 `UNCONFIRMED`。大型制品普通运行按 `DEC-ARTIFACT-INTEGRITY-01` 只核对路径、文件名与字节数，完整 SHA 仅在准入/变化/最终组包事件执行并复用记录。
