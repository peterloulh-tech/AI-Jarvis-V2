...
任务 26 动态纠正（2026-08-12）：锁定 V-C01 GGUF 的 `general.architecture=Qwen3VLForConditionalGeneration`，而锁定 b10369 只识别 `qwen3vl`，因此加载即失败，分类为 `CONFLICT/BLOCKED`；不得偷换旧 GGUF 或 runtime。V-C02 在 Mac/Metal 完成 9 次原生 1/2/3 图单请求，JSON/schema 9/9、完整契约 0/9，分类为 `FAIL`。两候选均未通过任务 26 硬门，任务 27 不得启动；Windows/NVIDIA 与 8/12GB 动态项仍为 `UNCONFIRMED`。大型制品普通运行按 `DEC-ARTIFACT-INTEGRITY-01` 只核对路径、文件名与字节数，完整 SHA 仅在准入/变化/最终组包事件执行并复用记录。
