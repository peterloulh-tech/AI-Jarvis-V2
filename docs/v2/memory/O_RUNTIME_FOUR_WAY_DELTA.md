...
| # / 段 | Original | Official Locked | Task23 | V2 current | 对齐、分类、复用与输出闭环影响 |
|---|---|---|---|---|---|
| 1 Screen / Vision capture | `DxgiDesktopCapture` 捕获桌宠所在显示器，失效重建并可退 GDI | C API只消费可解码图片路径；每个 push 可带一张图 | 读取固定 JPG；无屏幕采集 | 与 Original 相同 | PoC 合理绕过采集；正式接入复用 DXGI/GDI 但需显式源选择，`MODIFY`。不是当前 fragment 闭合候选。证据：`native/src/windows/dxgi_capture.cpp::DxgiDesktopCapture`；`omni.h::OmniDuplexFrame.img_fname` |
...
| 3 Audio conversion | `downmix_mono` 可直接复用；逐块 `resample_linear` 无跨块相位 | `audition_audio_preprocess` 通过 miniaudio 解码、下混并重采样到 16 kHz mono，再按 100 ms 补齐 | 不自行转换，交给 Locked runtime 解码 3 秒 WAV | 先 downmix/resample，再写 16 kHz float WAV；runtime 再解码 | Task23 路线是官方直接能力，非缺失；正式实时采集仍应 `DIRECT` 复用 downmix/window、`MODIFY` 为有状态 resampler。现场 73/73 result成功，不支持 decode failure 为当前主因。证据：`audio.cpp`；`audition.cpp::decode_audio_from_buf/audition_audio_preprocess` |
| 4 Frame / audio packaging | `RealOmniRuntime` 将 BGRA/float PCM 临时落盘 BMP/WAV | Locked frame ABI 就是 WAV/JPG/BMP 路径 + `max_slice_nums` + `user_seq` | 直接传已打包 JPG/WAV，`max_slice_nums=-1` | 与 Original 相同，生成临时 BMP/WAV并按 result 删除 | Task23 对 Locked ABI 对齐；正式临时原始媒体落盘有隐私/异常清理风险，`MODIFY`。不是当前 fragment 闭合候选。证据：`omni_runtime.cpp::write_bgra_bmp/write_float_wav/push_duplex`；`poc_main.cpp::run_session` |
...
| 11 Text / result extraction | Adapter取 C result；Worker仅在 speak 时发 text；Python再清洗/去重 | runtime token层解析 LISTEN/SPEAK，剥离控制 token；`OmniDuplexFrameResult.text` 明确定义为该帧文本片段；no-TTS仍填 text queue/result text | 每个 SPEAK fragment 到达即独立做完整 JSON parse，无跨 round buffer；原始 text 与 `ok` 都写 JSONL | 直接传 text；下游 `_clean_duplex_message` 和相似去重可抑制已发生的 speak | Task23 的逐 fragment 完整 JSON validation 是 `MISSING/MODIFY`；Locked 每帧最多 26 个 speak token，完整结构可跨多个 frame。C result 没有 EOS/finished 字段，测试层须按连续成功 SPEAK 聚合并记录 completion 依据。证据：`omni.h::OmniDuplexFrameResult`；`omni.cpp::duplex_do_decode/duplex_session_decode_worker_func`；`poc_main.cpp::validate_batches/run_session` |
...
- Task23 增加官方 Locked 参考的约 1 秒决策单元诊断；保留同一总体素材顺序、模型、prompt和 C API。拆片会同时改变 audio packaging、vision push 与静音分布，因此只能称 `1Hz official-alignment diagnostic`，不是严格单变量 A/B。
...
- **V2 startup guard**：`RealOmniRuntime::start_duplex` 把 Locked 默认 3 改成 1，没有本轮证据支持该 Delta。Task23绕过 adapter，且现场已确认第 4 轮后进入自主判断。
...
- 现场完整运行 `O10-P23-20260809-100451-WIN-01` 共 73 条 `duplex_result`，69 LISTEN、4 SPEAK，且 73 条均为 `ok:true`；failure-as-LISTEN 不再是该运行的主要候选。
...
## 12. 2026-08-10 现场证据后的候选排序

不强行宣布最终根因；当前按源码与 `O10-P23-20260809-100451-WIN-01` 排序：

1. **HIGH — streaming fragment 聚合 / completion boundary**：round 4～7 的 4 个连续 SPEAK 顺序拼接为同一 multi-batch JSON 前缀。Locked `text` 是每帧 fragment，且每帧最多 26 个 speak token；Task23 却逐 fragment 完整 parse、没有聚合 buffer。C result 只回传 sequence/frame/ok/is_speak/text/latency，没有 EOS/finished；内部 `__END_OF_TURN__` 在每帧 text 收集时被剔除，且 chunk-limit 分支也会结束该 decode，不能作为现有 EXE 可读取的完整 JSON 边界。
2. **HIGH — generation round / cadence 不足**：20 秒配置按 `offset<=cutoff` 只选择 0～18 秒的 7 个 push，前三个是 startup guard，恰剩 4 个自主 round；现场恰有 4 个连续 SPEAK fragment，和“窗口在 JSON 闭合前耗尽”相符但尚未构成最终因果证明。3 秒 cadence 已真实 SPEAK，不能再写成“模型无法 SPEAK”。
3. **MEDIUM — strict three-batch prompt**：它没有阻止 SPEAK，且模型已开始遵循 JSON/style 结构；但其复杂度和长度仍可能增加跨 round 闭合所需 token，尚无 prompt 单变量证据。
4. **LOW — timeout / drain**：73 条结果均成功；正常路径按 selected push 数等待并消费 1:1 FIFO 后才 `session_end`。只有出现 `result_wait_failed` 的失败路径才可能漏掉 drain 后未 pop 的 done result；增加未提交的新 generation round 也不是 drain 能完成的。
5. **LOW — failed result success gate**：harness 的确缺少先看 `ok` 再计 LISTEN/SPEAK 的 gate，但该完整运行 73/73 `ok:true`，不支持它作为当前主要原因。

已基本排除为当前首因：前三次 guard无限循环、`is_speak/is_listen`取反、TTS关闭和频繁 session重建。Task23单 case分别是默认 guard只用三次、映射正确、no-TTS text可用、无 context 重建。

## 13. Minimum Change Recommendation

`NEXT BEST ACTION`：复用同一 EXE/model/runtime/prompt，先跑两个测试层诊断：O-CASE-A 保持 3 秒输入并聚合连续 SPEAK fragment、把观察窗口延至完整 33 秒；O-CASE-B 使用同一总体内容的 33×1 秒切片，明确标记为 `1Hz official-alignment diagnostic`。不改 V2产品、不升级或重建 runtime。

`tools/aijarvisv2-23/AIJARVISV2-23-NEXT-WIN-DELTA/` 已提供独立 runner、离线聚合器、33×1 秒输入和零 GPU 自检；现有 `poc_main.cpp` 不需要修改。动态结论仍只由下一次 Windows/NVIDIA 真机产生。
