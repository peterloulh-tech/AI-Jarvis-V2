...
AIJARVISV2-19 已分离审计 V1 代码、vendor、MiniCPM 权重和桌面依赖的商业/离线分发条件并进入 `in_review`，见[任务卡](tasks/AIJARVISV2-19.md)。
...
AIJARVISV2-19 评审期间继续使用当前会话；不得自行改为 `done`，不得启动 AIJARVISV2-20。后续实际提取必须同时指向固定提交和具体符号/路径；vendor/model/依赖变化必须重新通过许可门。
