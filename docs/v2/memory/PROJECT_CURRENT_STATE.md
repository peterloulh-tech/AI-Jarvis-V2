...
AIJARVISV2-11 当前仍为 `backlog`，本项目记忆会话未启动它。后续若执行任务11，必须使用新 Codex 会话，先读其 Dashi 内容、直接依赖卡 [AIJARVISV2-8](tasks/AIJARVISV2-8.md)，再按需读 [`e2e-acceptance-matrix.md`](../requirements/e2e-acceptance-matrix.md) 的 V01～V10。
...
当前业务交付基线为 AIJARVISV2-10 提交 `ba28c363e44b986812a619620649bea390e5008b`。项目记忆在其上以独立分支维护，不改变该业务交付。
...
- O 模式固定 225/350/500 字预算与 12/16/20 秒硬超时；O 始终单模型、单持续会话、单文字生成流。V 超时和重启保护仍待实测/评审。
...
下一业务任务开始前先回读 Dashi，避免从编号顺序推定可启动状态。后续文档必须复用已有追踪、度量、证据和记录入口；发现与冻结基线冲突时记录评审，不直接改范围。任务进入 `in_review` 前同步任务卡，任务记忆未收口不得改为 `done`。
