...
现有资产还存在两项无法靠运行参数补齐的验收缺口：上游 `test-duplex` 使用固定 system prompt，且控制台只保留最多 60 字的文本预览，没有三批 JSON 提示、完整原文保存或逐批校验；V1 `native/src/omni_runtime.cpp` 的双工提示要求单句文本并会查找默认参考音频，且已被任务 22 明确禁止作为 O-C01 替代。任务 23 没有修改这些受保护实现。
...
在合格环境复跑时必须先完成以下入口，任一失败即保留原始日志并停止：

1. 在 Windows 11 x64/NVIDIA 16GB 独占机准备固定三项 GGUF，逐文件匹配任务 22 的大小与 SHA-256，并把许可材料和 O-IN-07 授权记录与运行 ID 绑定。
2. 从固定 vendor/patch 构建独立 CUDA 子进程；增加任务 23 专用的完整 JSONL 输出与三批校验入口，硬关闭 TTS、空参考音频，并用文件访问记录证明 TTS/token2wav 权重及默认参考音频读取为 0。
...
| 候选 | 本次处置 | 依据 |
|---|---|---|
| O-C01 | 保持唯一可复跑候选，但任务 23 当前为`阻塞` | 固定身份和四接口静态成立；模型/许可包、O-IN-07、合格硬件、专用三批记录入口及动态证据缺失 |
