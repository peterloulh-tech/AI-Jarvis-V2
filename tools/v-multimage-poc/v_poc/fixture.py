...
def build_fixtures(output_dir: Path) -> dict:
    ...
    for scene in ("calm", "ordinary", "highlight"):
        for image_count in (1, 2, 3):
            ...
            group_gold = dict(gold[scene])
            if image_count == 1 and scene == "ordinary":
                group_gold["required_event_terms"] = ["蓝"]
            elif image_count == 1 and scene == "highlight":
                group_gold["required_event_terms"] = ["红", "黄"]
            groups.append(
                {
    ...
    manifest = {
        "schema_version": 1,
        "fixture_version": "AIJARVISV2-26-synthetic-v2",
        "license": {
            "spdx": "CC0-1.0",
